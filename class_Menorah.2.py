...
class Menorah:
    ...
    def is_during_lighting_time(self): # after a candlighting time but before sunrise. FIX for SHABBOS
                                       # returns zero candles if not in this time period.
        for i, cl in enumerate(self.cl_times):
            if (cl < self.now_tz < self.sr_times[i+1]): # actually, next sr; this will be off by a few minutes
                print (i+1, ' candles') # need to add one?
                return True
            else:
                return False
    ...
    def is_waiting_for_lighting_time(self): # It's after a sunrise but before candlighting time.  Find that time. Already corrected for shabbos
        for i, value in enumerate(self.cl_times):
            if value > self.now_tz:
                return (i+1, value)
    ...
    def when_to_light_next(self):
        ...
        if self.is_now_before_first_candle():
            ...
            return 1
        if self.is_now_after_last_sunrise():
            print ('generate next year\'s lighting times; pause until then')
            return 0
        if self.is_during_lighting_time():
            print ('light ', 111, 'candles now')
            return 111
        candles = self.is_waiting_for_lighting_time()
        print ('light ', candles[0], ' candles at', candles[1])
